...
function parse_wpex_timeline_func($atts, $content){
	...
	$hide_img 		= isset($atts['hide_img']) ? $atts['hide_img'] : isset($atts['hide_thumb']) ? $atts['hide_thumb'] : '';
	...
	wp_enqueue_script( 'wpex-timeline', WPEX_TIMELINE.'js/template.min.js', array( 'jquery' ),'3.4' );
	if($the_query->have_posts()){?>
    	<div class="wpex-timeline-list <?php echo $css_class;?>" id="timeline-<?php echo esc_attr($ID);?>" data-animations="<?php echo esc_attr($animations);?>" data-rtl="<?php echo esc_attr($wpex_rtl_mode)?>">
