jQuery(window).on('elementor/frontend/init',function(){
	elementorFrontend.hooks.addAction('frontend/element_ready/pr-maps.default',function($scope,$){
        var mapElement = $scope.find('.pr_maps_map_height');
        var mapSettings = mapElement.data('settings');
        var mapStyle = mapElement.data('style');
        steMap = newMap(mapElement,mapSettings,mapStyle);
        ...
        function add_marker( pin, map ,autoOpen, hoverOpen, hoverClose ) {
            ...
            if( pin.find('.pr-maps-info-title').html() || pin.find('.pr-maps-info-desc').html() )
            {
                // create info window
